...
namespace ProjectAERP.Engine.Document
{
    public class CreateDocGIByLN : BaseEngine<CreateDocGIByLN.TReq, CreateDocGIByLN.TRes>
    {
        ...
        private void ConverterUnit(AMWLogger logger, ERPWHInbound.items wh_d, ams_SKUMaster sku, VOCriteria buVO)
        {
            var dataUnit = AWMSEngine.ADO.DataADO.GetInstant().SelectBy<ams_UnitType>(
                new SQLConditionCriteria[] {
                    new SQLConditionCriteria("Code",wh_d.inventory_unit.ToUpper(), SQLOperatorType.EQUALS),
                    new SQLConditionCriteria("Status",1,SQLOperatorType.EQUALS)
                }, this.BuVO).FirstOrDefault();

            var inventoryID = dataUnit == null ? null : StaticValue.UnitTypes.First(x => x.Code == wh_d.inventory_unit.ToUpper()).ID;
            if (inventoryID == null)
            {
                var unitID = AWMSEngine.ADO.DataADO.GetInstant().Insert<ams_UnitType>(this.BuVO, new ams_UnitType()
                {
                    Code = wh_d.inventory_unit.ToUpper(),
                    Name = wh_d.inventory_unit.ToUpper(),
                    ObjectType = StorageObjectType.PACK,
                    Status = EntityStatus.ACTIVE
