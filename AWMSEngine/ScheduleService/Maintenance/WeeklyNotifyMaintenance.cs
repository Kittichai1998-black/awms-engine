...
{
    public class WeeklyNotifyMaintenance : BaseNotification
    {
        ...
        protected override EmailFormat GetMessageEmail(dynamic reqVO, dynamic data)
        {
            ...
            {
                Body = msg,
                Subject = "Weekly Maintenance Plan"
            };
            return res;
        }
        ...
        {
            List<WeeklyMaintenancePlan.TRes> maintenance = AMWUtil.Common.ObjectUtil.DynamicToModel<List<WeeklyMaintenancePlan.TRes>>(message);
            ...
            if (notifySend == NotifyPlatform.Line)
            {
                ...
                {
                    strBody.Append($"{i}) {x.Name} | Date : {x.NextDateMaintenance} | Remain Days : {x.TotalDays}").AppendLine();
                    i++;
                });
            }
            else
            {
                ...
                strBody.Append("<th style=\"border:1px solid black\">Remain Day</th>");
                strBody.Append("</tr>");
                maintenance.ForEach(x =>
                {
                    strBody.Append("<tr>");
                    ...
                    strBody.Append("<td style=\"border:1px solid black\">" + x.TotalDays + "</td>");
                    strBody.Append("</tr>");
                    i++;
                });
