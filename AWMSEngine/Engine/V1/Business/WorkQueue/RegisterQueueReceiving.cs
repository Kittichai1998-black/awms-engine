    ...
    public class RegisterQueueReceiving : BaseQueue<RegisterQueueReceiving.TReq, WorkQueueCriteria>
    {
        ...
        protected override WorkQueueCriteria ExecuteEngine(TReq reqVO)
        {
            ...
            else if (mapsto.eventStatus == StorageObjectEventStatus.AUDITING || mapsto.eventStatus == StorageObjectEventStatus.AUDITED)
            {
                ...
                foreach (long docID in docItems.GroupBy(x => x.Document_ID).Select(x => x.Key)) {
                    var docItemDistos = ADO.DocumentADO.GetInstant().ListItemAndDisto(docID, this.BuVO);
                    foreach(var docItemDisto in docItemDistos)
                    {
                        docItemDisto.DocItemStos.ForEach(disto =>
                        {
                            if(disto.Status == EntityStatus.INACTIVE)
                            {
                                disto.Quantity = disto.Quantity ?? 0;
        ...
        private List<amt_DocumentItem> ProcessReceiving(StorageObjectCriteria mapsto, TReq reqVO)
        {
            ...
            foreach (var souWarehouse in mapstoTree
                                                    .Where(x => x.type == StorageObjectType.PACK && x.eventStatus == StorageObjectEventStatus.NEW)
                                                    .GroupBy(x => x.options)
