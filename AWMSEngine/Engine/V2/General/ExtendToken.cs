...
namespace AWMSEngine.Engine.V2.General
{
    public class ExtendToken : BaseEngine<ExtendToken.TReqModel, TokenObject>
    {
        ...
        protected override TokenObject ExecuteEngine(TReqModel reqVO)
        {
            ...
            tokenInfo.BodyDecode.exp = DateTime.Now.AddHours(this.StaticValue.GetConfigValue(ConfigCommon.TOKEN_EXPIRE_HR).Get2<int>());
            tokenInfo.BodyDecode.extend = DateTime.Now.AddHours(this.StaticValue.GetConfigValue(ConfigCommon.TOKEN_EXTEND_HR).Get2<int>());
