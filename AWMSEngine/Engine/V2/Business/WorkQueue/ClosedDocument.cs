...
namespace AWMSEngine.Engine.V2.Business.WorkQueue
{
    public class ClosedDocument : BaseEngine<List<long>, List<long>>
    {
                        ...
                        if (docs.EventStatus == DocumentEventStatus.CLOSING)
                        {
                            ...
                            else
                            {
                                ...
                                foreach (var di in distoGroupByWQ)
                                {
                                    ...
                                    {
                                        UpdateStorageObjectReceived(di.WorkQueueID, di.DiSTOs);
                                    }
                                    else
                                    {
        ...
        private void UpdateStorageObjectReceived(long? wqID, List<amt_DocumentItemStorageObject> distos)
        {
        ...
        private void UpdateStorageObjectIssued(List<amt_DocumentItemStorageObject> distos)
        {
            distos.ForEach(disto =>
            {
                ...
                {
                    var stoDes = AWMSEngine.ADO.DataADO.GetInstant().SelectByID<amt_StorageObject>(disto.Sou_StorageObject_ID, this.BuVO);
                    var done_des_event_status = ObjectUtil.QryStrGetValue(stoDes.Options, OptionVOConst.OPT_DONE_DES_EVENT_STATUS);
                    ...
                    if (stoDes.ParentStorageObject_ID == null)
                    {
                        if (done_des_event_status == null || done_des_event_status.Length == 0)
                        {
                            ADO.StorageObjectADO.GetInstant().UpdateStatus(disto.Sou_StorageObject_ID, StorageObjectEventStatus.PICKING, 
                                EntityStatus.ACTIVE, StorageObjectEventStatus.PICKED, this.BuVO);
                             
                        }
                        else
                        {
                            StorageObjectEventStatus eventStatus = (StorageObjectEventStatus)Enum.Parse(typeof(StorageObjectEventStatus), done_des_event_status);
                                ADO.StorageObjectADO.GetInstant().UpdateStatus(stoDes.ID.Value, StorageObjectEventStatus.PICKING, EntityStatus.ACTIVE, eventStatus, this.BuVO);
                            RemoveOPTEventSTO(stoDes.ID.Value, stoDes.Options, this.BuVO);
