...
namespace AWMSEngine.Engine.V2.Business.WorkQueue
{
    public class DoneQ : BaseEngine<DoneQ.TReq, List<long>>
    {
            ...
            if (queueTrx.Des_Warehouse_ID == _warehouse.ID.Value)
            {
                ...
                else if (queueTrx.IOType == IOType.OUTPUT && docs.DocumentType_ID != DocumentTypeID.AUDIT)
                {
                    ...
                    if (docItems.Count > 0)
                    {
                        ...
                        if(stoList2 == null | stoList2.Count() == 0)
                        {
                ...
                ManageWQ(reqVO, queueTrx, docItems);
            }
        ...
        private void UpdateSTOPack(StorageObjectCriteria sto)
        {
            var updSto = new StorageObjectCriteria();
            updSto = sto.Clone();
            updSto.parentID = null;
            updSto.parentType = null;
            updSto.mapstos = null;
            ...
            docItems.ForEach(docItem =>
            {
                if (docItem.Quantity == null)
                {
                    stoList.ForEach(sto =>
                    {
                        ...
                        {
                            disto.Des_StorageObject_ID = disto.Sou_StorageObject_ID;
                    ...
                    ADO.StorageObjectADO.GetInstant().UpdateStatus(stos.id.Value, StorageObjectEventStatus.PICKING, null, StorageObjectEventStatus.REMOVED, this.BuVO);
                                ...
                                {  //500 < 1000
                                   ...
                                   //ให้หักqty ออกจากstoเดิม ส่วนที่เหลือเป็น Received 
                                    ...
                                    {   //เบิกของหมด เปลี่ยนสภานะเป็น PICKING
                                        ...
                                        UpdateSTOPack(updSto);
                                        disto.Des_StorageObject_ID = updSto.id;
                                        disto.Status = EntityStatus.ACTIVE;
                                        ADO.DocumentADO.GetInstant().UpdateMappingSTO(disto.ID.Value, updSto.id, null, null, EntityStatus.ACTIVE, this.BuVO);
        ...
        private void ManageWQ(TReq reqVO, SPworkQueue queueTrx, List<TDocItems> docItems) 
        {
            if (queueTrx.EventStatus == WorkQueueEventStatus.WORKED || queueTrx.EventStatus == WorkQueueEventStatus.WORKING)
            {
                ...
                if (queueTrx.IOType == IOType.OUTPUT)
                {
                    if (docItems.Count > 0)
                    {
                        docItems.ForEach(docItem =>
                        {
                            docItem.DocItemStos.FindAll(x => x.WorkQueue_ID == queueTrx.ID.Value).ForEach(disto =>
                            {
                                var getArea = new MoveStoInGateToNextArea();
                                var baseSTO = ADO.DataADO.GetInstant().SelectBy<amt_StorageObject>(new SQLConditionCriteria[]
                                   {
                                        new SQLConditionCriteria("ID", queueTrx.StorageObject_ID.Value, SQLOperatorType.EQUALS),
                                        new SQLConditionCriteria("Status", EntityStatus.ACTIVE, SQLOperatorType.EQUALS)
                                   }, this.BuVO);
                                if (baseSTO == null || baseSTO.Count() == 0)
                                {
                                    var treq = new MoveStoInGateToNextArea.TReq()
                                    {
                                        stoID = disto.Des_StorageObject_ID.Value,
                                        stoType = StorageObjectType.PACK
                                    };
