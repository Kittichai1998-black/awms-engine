...
namespace AWMSEngine.ADO.WMSDB
{
    public class WaveADO : BaseMSSQLAccess<WaveADO>
    {
        ...
        {
            var wave = ADO.WMSDB.DataADO.GetInstant().SelectByID<amt_Wave>(waveID, buVO);
            wave.WaveSeqs = ADO.WMSDB.DataADO.GetInstant().SelectBy<amt_WaveSeq>(new SQLConditionCriteria[]{
        ...
        {
            amt_WaveSeq waveSeq = ADO.WMSDB.DataADO.GetInstant().SelectByID<amt_WaveSeq>(waveSeqID, buVO);
            return this.GetWaveAndSeq(waveSeq.Wave_ID, buVO);
        }
        public amt_Wave GetWaveAndSeq(long waveID, VOCriteria buVO)
        {
            amt_Wave wave = ADO.WMSDB.DataADO.GetInstant().SelectByID<amt_Wave>(waveID, buVO);
            wave.WaveSeqs = ADO.WMSDB.DataADO.GetInstant().SelectBy<amt_WaveSeq>(new SQLConditionCriteria[]
            {
                new SQLConditionCriteria("Wave_ID", waveID, SQLOperatorType.EQUALS),
