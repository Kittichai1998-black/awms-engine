...
{
    public class DocumentADO : ADO.WMSDB.BaseMSSQLAccess<DocumentADO>
    {
        ...
        public amt_DocumentItem CreateItem(amt_DocumentItem docItem, VOCriteria buVO)
        {
            ...
            /*docItem.StorageObjectIDs = ADO.WMSDB.DataADO.GetInstant()
        ...
        public amt_Document Get(long docID, VOCriteria buVO)
        {
            ...
            var res = ADO.WMSDB.DataADO.GetInstant().SelectBy<amt_Document>(whares.ToArray(), buVO);
            ...
            string orderNo, string batch, string lot, string options, VOCriteria buVO)
        {
            ...
            var res = ADO.WMSDB.DataADO.GetInstant().SelectBy<amt_DocumentItem>("amv_DocumentItem", "*", null, whares.ToArray(), null, null, null, buVO);
            return res;
        }
        public List<amt_DocumentItem> ListItem(long docID, VOCriteria buVO)
        {
            ...
            var res = ADO.WMSDB.DataADO.GetInstant().SelectBy<amt_DocumentItem>(whares.ToArray(), buVO);
            return res;
        }
        ...
        {
            var res = ADO.WMSDB.DataADO.GetInstant().SelectByID<amt_DocumentItem>(docItemID, buVO);
            var resSto = ADO.WMSDB.DataADO.GetInstant().SelectBy<amt_DocumentItemStorageObject>(
        ...
        public List<SPOutCountStoInDocItem> CountStoInDocItems(IEnumerable<long> docItemIDs, EntityStatus[] status, VOCriteria buVO)
        {
            ...
            var res = ADO.WMSDB.DataADO.GetInstant().SelectBy<SPOutCountStoInDocItem>(
        ...
        public List<amt_DocumentItem> ListItemCanMapV2(
            ...
            string orderNo, string batch, string lot, string options, VOCriteria buVO)
        {
            var docItems = ADO.WMSDB.DocumentADO.GetInstant()
            ...
            var countStoInDocItems = ADO.WMSDB.DocumentADO.GetInstant().CountStoInDocItems(docItems.Select(x => x.ID.Value).ToList(), buVO);
        ...
        public List<amt_DocumentItem> ListItemByWorkQueueDisto(long workQueueID, VOCriteria buVO)
        {
            ...
            var docItems = ADO.WMSDB.DataADO.GetInstant().SelectBy<amt_DocumentItem>(new SQLConditionCriteria[]
            {
        ...
        /*public List<amt_Document> ListAndRelationSupper(List<long> childDocumentIDs, VOCriteria buVO)
        {
            ...
            childDocumentIDs.ToList().ForEach(docID => {
                var doc = ADO.WMSDB.DocumentADO.GetInstant().ListParentLink(docID, buVO);
            ...
            baseDocs.ForEach(x =>
            {
                var ids = ADO.WMSDB.DocumentADO.GetInstant().ListItem(x.ID.Value, buVO).Select(y => y.LinkDocument_ID.Value).ToList();
                docHIDs.AddRange(ids);
            });
            docHIDs = docHIDs.Distinct().ToList();

            List<amt_Document> docHs = ADO.WMSDB.DocumentADO.GetInstant().List(docHIDs, buVO);
            docHs.ForEach(docH =>
            {
                docH.ParentDocument = baseDocs.FirstOrDefault(x => x.ID == docH.ParentDocument_ID);
                docH.DocumentItems = ADO.WMSDB.DocumentADO.GetInstant().ListItemAndDisto(docH.ID.Value, buVO);
            });
        ...
        {
            ADO.WMSDB.DataADO.GetInstant().Insert<aml_SAPLog>(buVO, new KeyValuePair<string, object>[]
            {
        ...
        {
            return ADO.WMSDB.DataADO.GetInstant().SelectBy<aml_SAPLog>(new SQLConditionCriteria[]{
