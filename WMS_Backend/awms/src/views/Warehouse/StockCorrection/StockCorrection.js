...
class StockCorrection extends Component {
  ...
  async componentWillMount() {
    document.title = "Stock Correction : AWMS";
    ...
    CheckWebPermission("StockCor", dataGetPer, this.props.history);
  ...
  displayButtonByPermission(dataGetPer) {
    let check = true
    if (CheckViewCreatePermission("WarehouseSC_view", dataGetPer)) {
      check = true //แสดงข้อมูล
    }
    if (CheckViewCreatePermission("WarehouseSC_create", dataGetPer)) {
      check = false //แก้ไข
    }
  ...
  createListTable() {
    ...
    if (status) {
      let data = {
        "scanCode": this.state.barcode, "amount": 0, "action": 0,
    ...
    return condata.map((child, i) => {
      ...
      return <ul key={i} style={child.isFocus === true ? focus : focusf}>
        ...
        {child.mapstos.length > 0 ? null : <Input defaultValue={child.allqty} style={{ height: "30px", width: "60px", background: "#FFFFE0", display: "inline-block" }} max="" type="number"
          onChange={(e) => {
            this.ChangeData(e, child.id, child.code, e.target.value, parent);
