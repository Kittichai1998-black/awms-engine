    ...
    return (
      <div>
        <Workbook filename={this.state.filename} element={<DropdownItem>{iconxls} Excel</DropdownItem>}>
          <Workbook.Sheet data={this.props.data} name="Sheet 1">
            {this.props.column.map((item) => 
              item.accessor === 'Status' ? (<Workbook.Column label={item.Header} value={(row) => row.Status === 1 ? "Active" : "Inactive"} />) : (<Workbook.Column label={item.Header} value={item.accessor} />)
            )}
          </Workbook.Sheet>
        </Workbook>
      </div>
