...
class LoadingDocument extends Component {
  constructor(props) {
    ...
    this.state = {
      ...
      readonly: false,
      transportselect: {
        queryString: window.apipath + "/api/tes",
        t: "Document",
        q: '[{ "f": "Status", "c":"=", "v": 1}]',
        f: "ID,Code, Name",
        g: "",
  ...
  async componentWillMount() {
    document.title = "Loading Checklist : AWMS";
    //permission
    this.setState({ showbutton: "none" })
    let dataGetPer = await GetPermission()
    CheckWebPermission("Loading", dataGetPer, this.props.history);
    this.displayButtonByPermission(dataGetPer)
    //permission
  }
  //permission
  //   33	TransLC_view
  // 34	TransLC_create&modify

  displayButtonByPermission(dataGetPer) {
    let check = true
    if (CheckViewCreatePermission("TransLC_view", dataGetPer)) {
      check = true //แสดงข้อมูล
    }
    if (CheckViewCreatePermission("TransLC_create&modify", dataGetPer)) {
      check = false //แก้ไข
    }
    if (check === true) {
