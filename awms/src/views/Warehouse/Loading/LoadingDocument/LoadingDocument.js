...
class LoadingDocument extends Component{
      ...
      this.setState({pageID:values.ID, readonly:true,
        ...
        if(rowselect1.data._result.status === 0){
          ...
          this.setState({
            ...
            ActionDate: moment(rowselect1.data.document.actionTime).format("DD/MM/YYYY"),
            ActionTime: moment(rowselect1.data.document.actionTime).format("HH:mm"),
          })
  ...
  callPDF() {
    console.log("ID: " + this.state.pageID);
    let dataitems = [];

      API.get(window.apipath + "/api/wm/loading/conso?docID=" + this.state.pageID).then(res => {
        dataitems = res.data;
      }).then(() => {
    ...
    if (datalength <= 10) {
      ...
      html2canvas(input, { dpi: 300 })
        .then((canvas) => {
          ...
          pdf.setFontSize(12);
          pdf.text(470, 35, 'No. ' + this.state.issuedNo);
          pdf.addImage(imgData, 'JPEG', 28, 40);
          pdf.save(this.state.issuedNo + ".pdf");
