...
class LoadingManage extends Component {
  ...
  async componentWillMount() {
    document.title = "Loading Document : AWMS";
    //permission
    this.setState({ showbutton: "none" })
    let dataGetPer = await GetPermission()
    CheckWebPermission("LDD", dataGetPer, this.props.history);
    this.displayButtonByPermission(dataGetPer)
  }
  //permission
  // 31	TransLD_view
  // 32	TransLD_create&modify

  displayButtonByPermission(dataGetPer) {
    let check = true
    if (CheckViewCreatePermission("TransLD_view", dataGetPer)) {
      check = true //แสดงข้อมูล
    }
    if (CheckViewCreatePermission("TransLD_create&modify", dataGetPer)) {
      check = false //แก้ไข
    }
    if (check === true) {
    ...
    return (
      ...
      */}
        ...
        <TableGen column={cols} data={this.state.select} filterable={true}
          ...
          accept={false} defaultCondition={[{ 'f': 'DocumentType_ID', c: '=', 'v': 1012 }]} />
        <Card >
          <CardBody >
