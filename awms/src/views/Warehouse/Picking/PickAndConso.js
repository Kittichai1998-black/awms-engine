...
import {Input, Modal, ModalHeader, ModalBody, ModalFooter, Button, Card, CardBody, ButtonGroup } from 'reactstrap';
...
class PickAndConso extends Component{
  ...
  createDocumentItemList(data){
    return data.map((rowdata, index) => {
      return <ul key={index}>
        <span>{rowdata.eventStatus} //</span><span>{rowdata.code} : {rowdata.name}//</span>
  ...
  render(){
    ...
    return(
      <div>
        {this.createModal()}
        <AutoSelect data={this.state.autocomplete} multi={true} result={result => this.renderTable(result)}/>
        <ReactTable data={this.state.data} columns={cols} minRows={10}
        ...
        <div>
          ...
          <Button color="primary" style={{display:"inline"}}>Scan</Button>
          <Button color="danger" style={{display:"inline-block"}}>Clear Result</Button>
        </div>
        <Card>
          <CardBody>
            <ButtonGroup style={{margin:'0 0 10px 0'}}>
              ...
              <Button color="primary" onClick={() => this.selectMode(2)} active={this.state.rSelect === 2}>Consolidate</Button>
            </ButtonGroup>
            <div>
              <label>Picking : </label>
              <Input placeholder="Barcode" type="text" onChange={e => this.setState({pickingBarcode:e.target.value})} style={{display:"inline-block", width:"180px"}}/>
              <Input placeholder="Amount" type="text" onChange={e => this.setState({pickingAmount:e.target.value})} style={{display:"inline-block", width:"100px"}}/>
