  ...
  genNewInputText(dociID, chkFirstClick) {
    ...
    if (dataProcessItems.length > 0) {
      ...
      DocumentItemData.forEach((datarow) => {
        if (datarow.dociID === dociID) {
          dataProcessItems.push({
            docID: datarow.docID, dociID: datarow.dociID, itemCode: datarow.itemCode, itemName: datarow.itemName, item: datarow.item
    ...
    return <div className={datarow.type}>
      <ReactTable style={{ width: "100%" }} data={datarow.data.filter(x => x.stoPack !== null).length > 0 ? (datarow.data.filter(x => x.stoPack)) : ([{}])}
        ...
        columns={[{ accessor: "baseCode", Header: "Pallet" }
          ...
          , Header: "Qty"
        }, { Cell: (e) => <span>{e.original.stoPack ? (e.original.stoPack.baseUnitCode ? e.original.stoPack.baseUnitCode : "") : ""}</span>, Header: "unit" }]} />
    </div>
  }
