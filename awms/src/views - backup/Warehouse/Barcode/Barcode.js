...
import {Input, Form, FormGroup, Card, CardBody, CardHeader, Col, Dropdown, DropdownItem, 
  DropdownMenu, DropdownToggle, Button, Textbox } from 'reactstrap';
...
class SetBarcode extends Component{
  ...
  componentDidMount(){
    ...
    this.setState({barcode:values.barcode,width:setup.width, height:setup.height})
  }
  ...
  createBarcode(event){
    ...
    if(this.state.chkbar === true && this.state.chkqr === false){
      ...
      for(let j=0; j< this.state.row; j++){
        element_row.push(<FormGroup style={{margin:'0 5px 5px 0'}} style={{margin:'0'}} key={j}>{element_column}</FormGroup>);
      }
    }
      ...
      for(let i=0; i< this.state.column; i++){
        ...
        <CardBody style={{ padding :'10px'}}>
          <QRCode value={getbarcode} size={170} />
        </CardBody>
      </Card>)
      }
      ...
      for(let i=0; i< this.state.column; i++){
        ...
        <CardBody style={{ padding :'10px'}}>
          ...
          <Barcode value={getbarcode} width={1.7} height={120} fontSize={13}/>
        </CardBody>
      </Card>)
      }
