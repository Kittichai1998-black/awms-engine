...
namespace ProjectPanKan.Engine.Business
{
    public class ListBaseConsoCanLoading : BaseEngine<ListBaseConsoCanLoading.TReq, ListBaseConsoCanLoading.TRes>
    {
        ...
        {
            var docLoadItems = AWMSEngine.ADO.DataADO.GetInstant().SelectBy<amt_DocumentItem>("Document_ID", reqVO.docID, this.BuVO);
            ...
            docLoadItems.ForEach(x => {
                var rsi = AWMSEngine.ADO.StorageObjectADO.GetInstant().ListBaseInDoc(x.LinkDocument_ID, null, DocumentTypeID.GOODS_ISSUED, this.BuVO);
                var rsiList = rsi.JsonCast<List<TRes.DataItem>>();
                var docIssuedItems = AWMSEngine.ADO.DataADO.GetInstant().SelectBy<amt_Document>("ID", x.LinkDocument_ID, this.BuVO);
                rsiList.ForEach(y => {
