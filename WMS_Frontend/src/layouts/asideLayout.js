...
const Aside = (props) => {
  ...
  const [backgroundColorChild, setBackgroundColorChild] = useState(props.backgroundColorChild);
  ...
  return (
    <MuiThemeProvider theme={theme}>
      <div >
        ...
        <List component="nav" className={classes.root} style={{ backgroundColor: backgroundColor }}>
          {routes.map((x, idx) => {
            if (x.child) {
              return <div key={idx}>
                <ListItem divider button onClick={() => onHandleClickToggle(x.text)} >
                  {icon === true ? (x.icon === null ? "" : HomeIcon(x.icon)) : null}
                ...
                {x.child.map((y, idx2) => {
                  return <Collapse key={idx2} in={state.menuID === x.text ? state.toggle : false} timeout="auto" unmountOnExit>
                    <List component="div" disablePadding >
                      <ListItem style={{ backgroundColor: y.to !== props.history.location.pathname ? colorLink : backgroundColorChild }} divider button component={Link} to={y.to}>
                        <span className="cui-user" aria-hidden="true"></span>
                        {HomeIcon("")}
                        {iconChild === true ? (y.iconChild === null ? "" : HomeIcon(y.iconSub)) : null}
              ...
              return <ListItem key={idx} divider button component={Link} to={x.to}>
                  <ListItemText primary={t(x.text.trim())} />
                {/* <ListItemText primary={x.text} /> */}
              </ListItem>
            }
