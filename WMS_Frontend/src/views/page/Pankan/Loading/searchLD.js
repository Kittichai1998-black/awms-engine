...
export default props => {
  ...
  const getRedirect = data => {
    return (
      <div style={{ display: "flex", padding: "0px", paddingLeft: "10px" }}>
        {data.Code}
        <AmRediRectInfo
          api={"/loading/detail?docID=" + data.ID}
  ...
  const OnclickExportData = () => {
    ...
    Axios.post(
      window.apipath + "/v2/ExportQueryToFileServerAPI",
      dataExport
    ).then(res => {
      console.log(res);
  ...
  return (
    <div>
      ...
      <AmDocumentSearch
        ...
        buttonReject={true}
        buttonWorking={true}
        apiReject={"/v2/RejectedLDDocAPI"}
        apiWorking={"/v2/WorkingLDDocAPI"}
      />
    </div>
  );
};
