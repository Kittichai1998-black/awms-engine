...
export default props => {
  ...
  const headerCreates = [
    ...
    [
      { label: "Doc Status", type: "labeltext", key: "", texts: "New" },
      {
        ...
        type: "labeltext",
        key: "ref1",
        texts: "R6-เบิกสินค้าเพื่อจัดนับ",
        valueTexts: "R06"
      }
    ]
  ];
        ...
        <FormInline>
          ...
          <InputDiv>
            <AmInput
              ...
              onChange={value => {
                onChangeEditor(row.accessor, value, null);
              }}
            />
          </InputDiv>
        </FormInline>
            ...
            {row.value.map((x, idx) => {
              return (
                <FormControlLabel
                  ...
                  onChange={e =>
                    onChangeEditor(row.accessor, e.target.value, null)
                  }
  ...
  const CreateDocument = () => {
    let MVTgroup = dataSource
      .map(row => row.BWLVS)
      .filter((val, i, obj) => obj.indexOf(val) === i)
      .join();
    let MATgroup = dataSource
      .map(row => row.MATNR)
      .filter((val, i, obj) => obj.indexOf(val) === i)
      .join();
    let document = {
