...
const StorageObject = props => {
  ...
  const iniCols = [
    ...
    { Header: "Base Unit", accessor: "Base_Unit", width: 100 },
    {
      ...
      type: "number",
      Footer: true,
      Cell: e => getNumberSaleQty(e.original)
    },
  ...
  const getNumberSaleQty = value => {
    return comma(parseInt(value.SaleQty));
  };
  const getNumberQty = value => {
    return comma(parseInt(value.Qty));
  };
  ...
  const search = [
    ...
    { label: "Qty", field: "SaleQty", searchType: "input", placeholder: "Qty" },
    {
      label: "Unit",
      field: "Unit",
      searchType: "dropdown",
