...
import { AmPagination, AmFilterTable } from "../../components/table";
...
const UseDocQuery = (docTypeCode) => {
    var querys = {
        queryString: window.apipath + "/v2/SelectDataViwAPI",
        t: "Document",
        q: '[{ "f": "DocumentType_ID", "c":"=", "v": "' + docTypeCode + '"}]',
        f: "*",
        g: "",
        s: "[{'f':'ID','od':'desc'}]",
        sk: 0,
...
const AmDocumentSearch = props => {
    ...
    const QueryDocument = UseDocQuery(props.docTypeCode )
    const [query, setQueryObj] = useState(QueryDocument)
        ...
        filterValue.forEach(fdata => {
            if (fdata.customFilter !== undefined) {
                if (IsEmptyObject(fdata.customFilter)) {
                    res = QueryGenerate({ ...query }, fdata.field, fdata.value)
                } else {
                    res = QueryGenerate({ ...query}, fdata.field, fdata.value, fdata.customFilter.dataType, fdata.customFilter.dateField)
                }
            }
        });
        console.log(res)
