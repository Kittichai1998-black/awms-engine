      ...
      if (con.conditions !== undefined) {
        con.conditions.forEach(y => {
          if (y.custom !== undefined) {
            ...
            if (getCustom.enable) x[y.key] = getCustom.defaultValue;
          } else {
            if (x.enable) x[y.key] = y.defaultValue;
            else x[y.key] = false;
          }
        });
      } else {
      ...
      if (con.eventStatuses !== undefined) {
        ...
        con.eventStatuses.forEach(y => {
          if (y.custom !== undefined) {
            ...
            if (y.enable && y.defaultValue) arrEvenstatus.push(y.value);
            else arrEvenstatus.push(12);
          }
        });
...
const useDocumentData = (doc, conditions) => {
  ...
  useEffect(() => {
    if (doc.documentsValue !== undefined) {
      var newDoc = doc.documentsValue.find(
        x => !doc.documentListValue.map(y => y.document.ID).includes(x.ID)
      );
      if (newDoc !== undefined) {
        const documentItemQuery = {
          ...
          t: "DocumentItem",
          q:
            '[{ "f": "Status", "c":"<", "v": 2},{ "f": "Document_ID", "c":"=", "v": ' +
            newDoc.ID +
            "}]",
...
const ProcessQueueDetail = props => {
  ...
  const { documents, documentDetail, warehouse } = useContext(
    ProcessQueueContext
  );
  ...
  useEffect(() => {
    const onClickDialog = (key, cell) => {
      const doc = documents.documentListValue.find(
        x => x.document.ID === cell.original.Document_ID
      );
    ...
    const createCustomDialog = event => {
      ...
      if (processCondition.conditions !== undefined)
        btnObj.push(
          <EditIcon
            onClick={() => {
              onClickDialog("conditions", event);
            }}
            fontSize="small"
          />
        );
      ...
      if (processCondition.eventStatuses !== undefined)
        btnObj.push(
          <ListIcon
            onClick={() => {
              onClickDialog("eventStatuses", event);
            }}
            fontSize="small"
          />
        );
      if (props.percentRandom)
        btnObj.push(
          <ListIcon
            onClick={() => {
              onClickDialog("percentRandom", event);
            }}
            fontSize="small"
          />
        );
    ...
    var createCustomColumns = {
      Header: "Configs",
      sortable: false,
      width: 100,
      Cell: x => createCustomDialog(x)
    };
  ...
  const genDocumentHeader = doc => {
    ...
    for (let i = 0; i < row; i++) {
      for (let j = 0; j < calColumns; j++) {
        const field = columnsField.splice(
          i * columnSize,
        ...
        field.forEach(z =>
          renderColumns.push(
            <Grid item xs={calColumns}>
              {z.label} : {doc.document[z.accessor]}
            </Grid>
          )
        );
      }
    ...
    var setFieldHeader = fieldHeader.map((x, idx) => {
      return (
        <span key={idx} style={{ marginRight: "20px" }}>
          {x.label} : {doc.document[x.accessor]}
        </span>
      );
    });
            ...
            obj.push({
              ...
              component: (data, cols, key) => {
                ...
                if (condition.enable) {
                  ...
                  var findOrb = {
                    ...orb.find(y => y.fieldName === x.sortField)
                  };
                  return (
                    <FormInline>
                      <LabelH>{x.field} : </LabelH>
                      <AmDropdown
                        ...
                        onChange={(
                          ...
                          let findEdit = orb.findIndex(
                            y => y.fieldName === findOrb.fieldName
                          );
                      ...
                      <CheckboxCustom
                        ...
                        onClick={event => {
                          if (event.target.checked) {
                            if (
                              !data.docItem["eventStatuses"].includes(x.value)
                            ) {
                              data.docItem["eventStatuses"].push(x.value);
                            }
                          } else {
                            data.docItem["eventStatuses"] = data.docItem[
                              "eventStatuses"
                            ].filter(y => y !== x.value);
                          }
                        }}
                        defaultChecked={data.docItem["eventStatuses"].includes(
                          x.value
                        )}
                      />
                    </FormInline>
  ...
  const onClickProcessQueue = () => {
      ...
      .forEach(doc => {
        doc.docItems.forEach(docItem => {
          ...
          let processQueue = {
            ...
            skuCode: getOptions.palletcode
              ? null
              : docItem.Code
              ? docItem.Code
              : null,
            priority: docItem.priority ? docItem.priority : 2,
            useShelfLifeDate: docItem.useShelfLifeDate
              ? docItem.useShelfLifeDate
              : false,
            useExpireDate: docItem.useExpireDate
              ? docItem.useExpireDate
              : false,
            useIncubateDate: docItem.useIncubateDate
              ? docItem.useIncubateDate
              : false,
            ...
            percentRandom: props.percentRandom
              ? docItem.percentRandom !== undefined
                ? docItem.percentRandom
                : 100
              : null,
    ...
    if (areaEnable && IsEmptyObject(areaSelection)) {
      ...
      processQueueData["desASRSWarehouseCode"] = areaEnable
        ? warehouse.warehouseValue.Code
        : null;
      processQueueData["desASRSLocationCode"] = null;
      processQueueData["desASRSAreaCode"] =
        areaEnable && !IsEmptyObject(areaSelection) ? areaSelection.Code : null;
      processQueueData["processQueues"] = processQueueArr;

      Axios.post(window.apipath + "/v2/process_wq", processQueueData).then(
        res => {
          if (res.data._result.status !== 1) {
            ...
            if (process.length === 0) {
              ...
              let createResData = {};
              createResData["desASRSWarehouseCode"] = areaEnable
                ? warehouse.warehouseValue.Code
                : null;
              createResData["desASRSLocationCode"] = null;
              createResData["desASRSAreaCode"] =
                areaEnable && !IsEmptyObject(areaSelection)
                  ? areaSelection.Code
                  : null;
              createResData["processResults"] = process;
              setProcessQueueData(res.data);
              if (!confirmState) setConfirmState(true);
            }
          }
        }
      );
      ...
      <AmEditorTable
        ...
        onAccept={(status, rowdata) => {
          ...
          if (rowdata.docItems !== undefined) {
            let findEdit = rowdata.docItems.findIndex(
              x => x.ID === rowdata.docItem.ID
            );
            if (status) {
      ...
      <Grid container>
        <Grid item xs="6">
          <AmButton
            ...
            styleType="delete"
            disabled={
              documents.documentListValue === undefined ||
              documents.documentListValue.length === 0
            }
            onClick={() => {
              documents.clearDocument();
            }}
...
const ConfirmDialog = props => {
  ...
  const onClickConfirm = () => {
    ...
    Axios.post(window.apipath + "/v2/wave_process_wq", confirmData).then(
      res => {
              ...
              {x.processResultItems.map(item => {
                ...
                return (
                  <FormInline>
                    <Grid container>
                      <Grid item xs="6">
                        <label>Code : {item.docItemCode}</label>
                      </Grid>
                      <Grid item xs="6">
                        <label>
                          Quantity : {item.baseQty + " " + getUnit}{" "}
                        </label>
                      </Grid>
                      {item.percentRandom !== null ? (
                        <Grid item xs="6">
                          <label>Percent : {item.percentRandom}</label>
                        </Grid>
                      ) : null}
                    </Grid>
                    <div style={{ width: "100%" }}>
