...
const useColumns = (props) => {
    ...
    useEffect(()=>{
        if(selection.length > 0){
            selection.forEach(x => {
                let element = document.getElementById("selection_" + x[props.primaryKey]);
    ...
    useEffect(()=>{
        ...
        if(props.selection){
            if(props.selectionType === "radio"){
                ...
                getColumns.unshift({
                    Header: (ele) => <input id="selectAll" checked={selectionAll.select} type="checkbox" onChange={(e)=> {
                            if(e.target.checked){
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps
    }, [props.columns, props.selection, props.selectionType, props.primaryKey, selectionAll, selection])
    ...
    const addSelection = (rowData, check) => {
        if(check){
            ...
            let uncheck = selection.filter(sel => sel[props.primaryKey] !== rowData[props.primaryKey])
...
const AmTableMaster = (props) => {
            ...
            if(props.sortable){
                if(row.sortable === undefined || row.sortable === true){
                    ...
                    if(sort !== undefined && sort !== null && sort !== {}){
                        if(row.accessor === sort.id){
                            if(sort.sortDirection === SortDirection.DESC)
                                orderBy = { id: row.accessor, sortDirection: SortDirection.ASC };
                            else
                                orderBy = { id: row.accessor, sortDirection: SortDirection.DESC };
                        }
                    }
                    ...
                    return <div style={{width:"100%"}} onClick={()=>{
                        ...
                        <span><Arrow style={{ transform: "rotate(-135deg)", WebkitTransform: "rotate(-135deg)"}}/></span> : 
                        <span><Arrow style={{ transform: "rotate(45deg)", WebkitTransform: "rotate(45deg)"}}/></span>}
                    </div>
                }
                else{
    ...
    const renderBodyCell = (row, dataIdx) => {
        ...
        return columns.map((column, idx) => {
            ...
            if(column.fixed){
                ...
                return <TableCell style={column.style === undefined ? props.cellStyleProps : column.style} 
                key={idx}>{column.Cell === undefined || column.Cell === null ? renderCellText(column, row[column.accessor]) : column.Cell(createCellData)}</TableCell>
            }
        });
    }
    ...
    const RenderCustomTopRight = () => {
        return <div style={{float:"right", marginBottom:"10px"}}>{props.renderCustomTopRight}</div>
    }

    const RenderCustomTopLeft = () => {
        return <div style={{float:"left", marginBottom:"10px"}}>{props.renderCustomTopLeft}</div>
    }
