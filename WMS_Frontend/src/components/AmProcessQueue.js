...
const styles = theme => ({
    ...
    fontIndi_0: {
        color: "#1b5e20", minHeight: '70px', paddingTop: '5px',
        fontSize: 'small', fontWeight: 'bold'
    },
    fontIndi_1: {
        color: "#01579b", minHeight: '52px', paddingTop: '5px',
        fontSize: 'small', fontWeight: 'bold'
    },
...
const AmProcessQueue = (props) => {
    ...
    }, [props.detailDocument, dataDetialdoc, detailsdata, props.onChangeDoc, localStorage.getItem("Lang")])
    ...
    const onHandleChangeDDLDocument = (value, dataObject, inputID, fieldDataKey, field, data) => {
        //setDataDocumentItem();
        if (value !== undefined && value !== null && dataObject.Code !== null) {
            setdocCodelink(dataObject.Code)
            setDocumentID(value)
            getDoumentItem(value);
            var filterDoc = dataDocument.filter(x => x["DocumentCode"] === dataObject.Code);
    ...
    const onHandleChangeDDLProcess = (value, dataObject, inputID, fieldDataKey, field, data) => {
        if (value !== undefined && value !== null) {
    ...
    const onHandleChangeDDLProcessAPI = (value, dataObject, inputID, fieldDataKey, field, data) => {
        if (value !== undefined && value !== null) {
    ...
    const getDoumentItem = (docID) => {
        let DocumentItem = {
            ...
            f: "ID,Code,RefID,Ref2,Batch,Ref1,Quantity,BaseQuantity,Options,SKUMaster_Name,OrderNo,Lot,BaseUnitType_Code,PackMaster_Code,UnitType_Name",
            g: "",
            s: "[{'f':'ID','od':'asc'}]",
            sk: 0,
            all: "",
    ...
    const getDataDetail = (type, key, values) => {
        if (type === "label") {
            return <div style={{ marginLeft: "10px" }}><label>{values}</label></div>
        }
    }
    ...
    const onHandleEditConfirm = (status, rowdata) => {
        if (status) {
            ...
            if (chkData.length > 0) {
                ...
                let sumQty = SumQty(dataSource[indexBtn], "BaseQuantity") + parseInt(rowdata.BaseQuantity, 10);
                var datsTB = dataSource[indexBtn]
                let BaseRow = parseInt(rowdata.BaseQuantity, 10)
                let dataBaseTb = (dataSource[indexBtn].map(x => { return x.BaseQuantity }))
    ...
    const getdataDDLSort = (value, dataObject, inputID, fieldDataKey, field, data, pair) => {
    ...
    const onChangeEditorSort = (field, rowdata, value, pair, dataPair) => {
        if (addData) {
            ...
            let editRowX = editDataSort.original ? { ...editDataSort.original } : { ...editDataSort };
            editRowX[field] = value;
            if (pair) {
                editRowX[pair] = dataPair;
            }
        ...
        return columnCondition.map(row => {
            return {
                ...
                "component": (data = null, cols, key) => {
                    return <div key={key}>
                        {getTypeEditor(row.type, row.Header, row.accessor, row.validate, row.idddls, row.fieldLabel, row.field, row.queryApi, data, cols, row, row.dataDDL)}
                    </div>
                }
            }
        })
    ...
    const editorListcolunmSort = () => {
        return props.columnSort.map(row => {
            return {
                ...
                "component": (data = null, cols, key) => {
                    return <div key={key}>
                        {getTypeEditorSort(row.type, row.Header, row.accessor, row.validate, row.idddls, row.fieldLabel, row.field, row.queryApi, data, cols, row, row.dataDDL)}
                    </div>
                }
            }
        })
    }
    const getTypeEditor = (type, Header, accessor, validate, idddls, fieldLabel, queryApi, field, data, cols, row, dataDDL) => {
        if (type === "input") {
            return <FormInline> <LabelH>{t(Header)} : </LabelH><InputDiv>
                <div ><AmInput
                    style={{ width: "300px" }}
                    id={cols.field}
                    validate={true}
                    ...
                    onChange={(ele) => { onChangeEditor(cols.field, data, ele, "") }}></AmInput></div>
            </InputDiv></FormInline>
            ...
            return <FormInline> <LabelH>{t(Header)} : </LabelH><InputDiv>
                ...
                ><AmInput
                    ...
                    </AmInput></div> </InputDiv></FormInline>
            ...
            return <FormInline> <LabelH>{t(Header)} :</LabelH><InputDiv>
                <div><AmDropdown
                    ...
                    onChange={(value, dataObject, inputID, fieldDataKey) => onChangeEditor(value, dataObject, inputID, fieldDataKey)}
                    ddlType={"search"}
                >
                </AmDropdown></div>
            </InputDiv></FormInline>
            ...
            return <FormInline> <LabelH>{t(Header)} : </LabelH>
                <InputDiv>
    ...
    const getTypeEditorSort = (type, Header, accessor, validate, idddls, fieldLabel, queryApi, field, data, cols, row, dataDDL) => {
        if (type === "input") {
            return <FormInline> <LabelH>{t(Header)} : </LabelH><InputDiv>
                <div > <AmInput
                    id={cols.field}
                    ...
                    regExp={validate ? validate : ""}
                    defaultValue={data ? data[cols.field] : ""}
                    onChange={(ele) => { onChangeEditorSort(cols.field, data, ele, row.pair) }}></AmInput></div>
            </InputDiv></FormInline>
        } else if (type === "inputnum") {
            return <FormInline> <LabelH>{t(Header)} : </LabelH><InputDiv>
                <div > <AmInput
                    ...
                    style={{ width: "100px" }}
                    type="number"
                    onChange={(ele) => { onChangeEditorSort(cols.field, data, ele, row.pair) }}>
                </AmInput></div> </InputDiv>
            </FormInline>
        } else if (type === "dropdown") {
            return <FormInline> <LabelH>{t(Header)} :</LabelH><InputDiv>
                <div ><AmDropdown
                    ...
                    ddlMinWidth={300} //��˹��������ҧ�ͧ���ͧ dropdown
                    valueData={valueText[idddls]} //��� value ������͡    
                    defaultValue={data !== {} && data !== null ? data[accessor] : ""}
                    onChange={(value, dataObject, inputID, fieldDataKey) => getdataDDLSort(value, dataObject, inputID, fieldDataKey, cols.field, data, row.pair)}
                    ddlType={"search"}
                >
                </AmDropdown></div>
            </InputDiv></FormInline>
            ...
            return <FormInline> <LabelH>{t(Header)} :</LabelH><InputDiv>
                <div> <AmDropdown
                    ...
                    queryApi={queryApi}
                    //defaultValue={data ? data : ""}               
                    onChange={(value, dataObject, inputID, fieldDataKey) => getdataDDLSort(value, dataObject, inputID, fieldDataKey)}
                    ddlType={"search"} //�ٻẺ Dropdown 
                /></div>
            </InputDiv></FormInline>
        }
    ...
    const ProcessDLL = () => {
        return props.ProcessQ.map(row => {
            return <div>
                {getTypeProcessQ(row.type, row.key, row.idddls, row.fieldLabel, row.queryApi, row.field, row.dataDDL, row.Label, row.defaultValue)}
            </div>
        })
    }

    const getTypeProcessQ = (type, key, idddls, fieldLabel, queryApi, field, dataDDL, Label, defaultValue) => {
        if (type === "dropdown") {
            return <FormInline> <LabelHDes>{t(Label)} :</LabelHDes><InputDiv>
                <AmDropdown
                    id={idddls}
                    placeholder="Select"
                    data={dataDDL}
                    ...
                    ddlMinWidth={300} //��˹��������ҧ�ͧ���ͧ dropdown
                    valueData={valueText[idddls]} //��� value ������͡                     
                    onChange={(value, dataObject, inputID, fieldDataKey) => onHandleChangeDDLProcess(value, dataObject, inputID, fieldDataKey, field)}
                    ddlType={"search"}
                >
                </AmDropdown>
            </InputDiv></FormInline>
            ...
            return <FormInline> <LabelHDes>{t(Label)} :</LabelHDes><InputDiv>
                <AmDropdown
                    ...
                    //defaultValue={data ? data : ""}               
                    onChange={(value, dataObject, inputID, fieldDataKey) => onHandleChangeDDLProcessAPI(value, dataObject, inputID, fieldDataKey, field)}
                    ddlType={"search"} //�ٻẺ Dropdown 
                />
            </InputDiv></FormInline>
        ...
        if (detailsdata === undefined) {
            return detailDocuments.map((x) => {
                return <Grid container spacing={24}>
                    {x.map(y => {
                        let syn = y.label ? " :" : ""
                        return <Grid xs={6} style={{ paddingLeft: "10px", paddingBottom: "10px" }}>
                            <FormInline>
                                <div style={{ paddingLeft: "10px", width: "200px" }}><LabelH>{t(y.label) + syn}</LabelH></div>
            ...
            return detailsdata.map((x) => {
                return <Grid container spacing={24}>
                    {x.map(y => {
                        return <Grid xs={6} style={{ paddingLeft: "10px", paddingBottom: "10px" }}>
                            <FormInline>
                                <div style={{ paddingLeft: "10px", width: "200px" }}><LabelH>{t(y.label)}</LabelH></div>
                                {getDataDetail(y.type, y.key, y.values)}
                            </FormInline>
                        </Grid>
                    })
    ...
    const addConditions = () => {
        var filterDocCode = dataQueue.filter(x => x["DataDocumentCode"] === dataDocument[dataDocument.length - 1].DocumentCode);
        if (filterDocCode.length === 0) {
            ...
            datasQ["DataDocumentCode"] = dataDocument[dataDocument.length - 1].DocumentCode;
            ...
            Axios.get(createQueryString(docQuery)).then(res => {
                let docSelection = res.data.datas.filter(x => {
                    return [...dataQueue].filter(y => y.DataDocumentCode == x.Code).length === 0;
                });
                setapiDoc(docSelection.map(x => ({ Code: x.Code, value: x.ID })))
            })
    ...
    const OnclickBackAddDocument = (doc, dataSc, dataSt, ) => {
        Axios.get(createQueryString(docQuery)).then(res => {
            let docSelection = res.data.datas.filter(x => {
                let dataInQueue = [...dataQueue].filter(y => y.DataDocumentCode === x.Code);
        ...
        datasDoc.forEach((a, idx) => {
            a.DataDocumentItem.forEach((y) => {
                ...
                if (a.DataSorting[0] !== undefined) {
                    a.DataSorting.filter(x => x.docItemID === y.ID).forEach((ds, dsIdx) => {
                ...
                let processQueuesz = {
                    ...
                    "baseQty": y.BaseqtyMax ? y.BaseqtyMax : y.BaseQuantity ? y.BaseQuantity : null,
                ...
                <div>
                    {
                        process.map((x, idx) => {
                            return <div><div> <FormInline>
                                <div style={{ marginLeft: "5px", paddingTop: "15px", paddingBottom: "10px", }}>
                                    <LabelH>{t('Document')} : {x.docCode}</LabelH></div>
                            </FormInline></div>
                                {
    ...
    const SumTables = (datasConfirms) => {
        ...
        return props.columnConfirm.filter(row => row.Footer === true).map(row => {
            return {
                accessor: row.accessor, sumData: sumFooterTotal(datasConfirms) + "/" + sumFooterTotalMax(datasConfirms)
            }
        })
    }
    ...
    const sumFooterTotalMax = (datasConfirms) => {
        ...
        if (sumVals === 0 || sumVals === null || sumVals === undefined || isNaN(sumVals)) {
            return '-'
        } else {
            return sumVals
        }
    ...
    const OnclickConfirmCleare = () => {
        ...
        setbodyDailogClear(<div style={{ width: "500px", height: "100px" }}>
            ...
            <FormInline><label>Confirm Clare Data</label></FormInline>
        </div>)
    }
        ...
        <div>
            <div>
                ...
                <AppBar position="static" color="default">
                    <Tabs
                        classes={{ indicator: classnames(classes.bigIndicator, classes['indicator_' + value]) }}
                        value={value}
                        onChange={handleChange}
                        scrollButtons="on" >
                        <Tab label={t("Add")} className={classes.fontIndi_0} icon={<LibraryAdd />} />
                        <Tab label={t("Queue")} className={classes.fontIndi_1} icon={<CheckBox />} />
                    </Tabs>
                </AppBar>
            </div>
            {value === 0 ? <Card style={{ overflow: "initial" }}>
                {value === 0 ?
                    <div style={{ marginTop: "10px", paddingBottom: "10px", background: "#eeeeee" }}>
                        <Grid container spacing={16}>
                            <Grid item>
                                <FormInline>
                                    <div style={{ marginLeft: "10px", marginTop: "10px", width: "150px" }}>
                                        <LabelH>Source Warehouse : </LabelH></div>
                                    <div style={{ marginTop: "10px", marginLeft: "8px" }}> <AmDropdown
                                        ...
                                        returnDefaultValue={true}
                                        defaultValue={props.Defaulwarehouse ? props.Defaulwarehouse : warehouseID ? warehouseID : ""}
                                ...
                                <FormInline>
                                    ...
                                    <div style={{ marginTop: "10px", marginLeft: "8px" }}> <AmDropdown
                                        ...
                                        onChange={(value, dataObject) => onHandleChangeDDLDocument(value, dataObject)}
                                        ddlType={"search"} //รูปแบบ Dropdown 
                                    />
                                    </div>
                                    <div style={{ marginLeft: "10px" }}>
                                        {documentID ?
                                            <AmRediRectInfo
                                                ...
                                                api={props.apidetail + documentID} history={props.history} >
                                            </AmRediRectInfo > : null}
                                    </div></FormInline></Grid></Grid></div> : null}
                        ...
                        {value === 0 ? <div>  {
                            [...DataDocumentItem].map((x, idx) => {
                                ...
                                columnConditionx.push(
                                    { Header: "", width: 110, Cell: (e) => e.original.ID < 0 ? <AmButton style={{ width: 100 }} styleType="info" disable={e.original.ID > 0 ? false : true} onClick={() => { setEditData(e); setDialog(true); setTitle("Edit"); }}>{t("Edit")}</AmButton> : null },
                                    { Header: "", width: 110, Cell: (e) => e.original.ID < 0 ? <AmButton style={{ width: 100 }} styleType="delete" disable={e.original.ID > 0 ? false : true} onClick={() => { onHandleDelete(e.original.ID, e.original, e, idx); }}>{t("Remove")}</AmButton> : null });

                                var columnSortx = [...columnSort]
                                columnSortx.push(
                                    { Header: "", width: 110, Cell: (e) => <AmButton style={{ width: 100 }} styleType="info" onClick={() => { setEditDataSort(e); setDialogSort(true); setTitleSort("Edit Sort"); }}>{t("Edit")}</AmButton> },
                                    { Header: "", width: 110, Cell: (e) => <AmButton style={{ width: 100 }} styleType="delete" onClick={() => { onHandleDeleteSort(e.original.ID, e.original, e, idx); }}>{t("Remove")}</AmButton>, });

                                if (x.BaseQuantity !== null && !qtyDocItem[idx] && x.BaseQuantity !== undefined) {
                                ...
                                if (x.Options !== null && x.Options !== undefined) {
                                    if (x.Options.split("=")[1] !== undefined && x.Options.split("=")[2] !== undefined && x.Options.split("=")[3] !== undefined) {
                                        var palletcode = x.Options.split("=")[1].split("&")[0];
                                        DataDocumentItem[idx]["palletcode"] = palletcode
                                        var locationcode = x.Options.split("=")[2].split("&")[0];
                                        DataDocumentItem[idx]["locationcode"] = locationcode
                                        var qtyrandom = x.Options.split("=")[3].split("&")[0];
                                        if (qtyrandom !== undefined)
                                            var ele = qtyrandom
                                        onChangeRandom(ele, idx, qtyrandom)
                                    } else if ((x.Options.split("=")[1] !== undefined && x.Options.split("locationcode=")[2] !== undefined && x.Options.split("qtyrandom=")[3] === undefined)) {
                                        var palletcode = x.Options.split("=")[1].split("&")[0];
                                        DataDocumentItem[idx]["palletcode"] = palletcode
                                        var locationcode = x.Options.split("=")[2].split("&")[0];
                                        DataDocumentItem[idx]["locationcode"] = locationcode

                                    } else if ((x.Options.split("=")[1] !== undefined && x.Options.split("locationcode=")[2] === undefined && x.Options.split("=")[2] !== undefined)) {
                                        var palletcode = x.Options.split("=")[1].split("&")[0];
                                        DataDocumentItem[idx]["palletcode"] = palletcode
                                    ...
                                    } else if ((x.Options.split("palletcode=")[1] === undefined && x.Options.split("=")[1] !== undefined && x.Options.split("=")[2] !== undefined)) {
                                        var locationcode = x.Options.split("=")[2].split("&")[0];
                                        DataDocumentItem[idx]["locationcode"] = locationcode
                                        var qtyrandom = x.Options.split("=")[3].split("&")[0];
                                        if (qtyrandom !== undefined)
                                    ...
                                    } else if ((x.Options.split("palletcode=")[1] !== undefined)) {
                                        var palletcode = x.Options.split("=")[1].split("&")[0];
                                        DataDocumentItem[idx]["palletcode"] = palletcode


                                    } else if ((x.Options.split("locationcode=")[1] !== undefined)) {
                                        var locationcode = x.Options.split("=")[1].split("&")[0];
                                        DataDocumentItem[idx]["locationcode"] = locationcode
                                        //onChangeRandom(ele, idx, qtyrandom)
                                    } else if ((x.Options.split("qtyrandom=")[1] !== undefined)) {
                                ...
                                return <div style={{ marginLeft: "10px", marginRight: "10px" }}><BorderAdd><Card>
                                    <AmButton styleType="add_clear" style={{ marginLeft: "10px" }}
                                        onClick={() => onclickToggel(idx)}>{x.Code}: {x.SKUMaster_Name}
                                        <ExpandLessIcon className={toggle[idx] ? classes.expand : classes.collapse} /></AmButton>
                                        ...
                                        {toggle[idx] === true ? <Paper elevation={4} className={classes.paper}>
                                            ...
                                            <FormInline>
                                                <div style={{ marginLeft: "15px", paddingTop: "10px" }}>
                                                    <LabelH>{t("Priority")} : </LabelH></div>
                                                <div style={{ marginLeft: "130px" }}> <AmDropdown
                                                    id="priority"
                                                    placeholder="Select"
                                                    data={props.priolity ? props.priolity : priolity()}
                                                    fieldDataKey="value"
                                                    fieldLabel={["label"]}
                                                    width={200} //��˹��������ҧ�ͧ��ͧ input
                                                    ddlMinWidth={100} //��˹��������ҧ�ͧ���ͧ dropdown
                                                    valueData={valueText} //��� value ������͡
                                                    defaultValue={x.PriorityDoc ? x.PriorityDoc : props.priolity ? 2 : defaultpriority}
                                                    // returnDefaultValue={true}
                                                    onChange={(value, dataObject) => Onchangepriolity(value, dataObject, idx)}
                                                    ddlType={"search"}
                                                >
                                                </AmDropdown>
                                                </div>
                                            </FormInline>
                                            {props.advanceCondition === true ? <FormInline style={{ marginLeft: "15px", paddingTop: "10px" }}>
                                                <LabelH>{t("Advance Condition")} : </LabelH>
                                                <FormInline>
                                                    <div> {props.fullPallet === true ? <AmCheckBox value="FullPallet" label="FullPallet" checked={true} onChange={(e, v) => onChangCheckboxConsFull(e, v, idx)}>></AmCheckBox>
                                                        : <div>
                                                            <FormInline>
                                                                <AmCheckBox value="ShelfLifeDate" label="ShelfLifeDate" checked={x.ShelfLife ? true : null} onChange={(e, v) => onChangCheckboxCon(e, idx)}>></AmCheckBox>
                                                                <AmCheckBox value="IncubateDate" label="IncubateDate" checked={x.Incubate ? true : null} onChange={(e, v) => onChangCheckboxCon(e, idx)}>></AmCheckBox>
                                                                <AmCheckBox value="ExpireDate" label="ExpireDate" checked={x.ExpiredDate ? true : null} onChange={(e, v) => onChangCheckboxCon(e, idx)}>></AmCheckBox>
                                                                <AmCheckBox value="FullPallet" label="FullPallet" checked={x.FullPallet ? true : null} onChange={(e, v) => onChangCheckboxCon(e, idx)}>></AmCheckBox>
                                                            </FormInline>
                                                        </div>}
                                            ...
                                            {props.status === true ? <FormInline style={{ marginLeft: "15px" }}>
                                                <LabelH>{t("Status")} : </LabelH>
                                                <FormInline>{props.receive === true ?
                                                    <AmCheckBox value="Receive" label="Receive" checked={true} onChange={(e, v) => onChangCheckboxConsRecieve(e, v, idx)}>></AmCheckBox>
                                                    : <div><FormInline>
                                                        <AmCheckBox value="Receive" label="Receive" checked={x.Recive ? true : null} onChange={(e, v) => onChangCheckboxStatus(e, idx)}>></AmCheckBox>
                                                        <AmCheckBox value="Block" label="Block" checked={x.Block ? true : null} onChange={(e, v) => onChangCheckboxStatus(e, idx)}>></AmCheckBox>
                                                        <AmCheckBox value="QC" label="QC" checked={x.QC ? true : null} onChange={(e, v) => onChangCheckboxStatus(e, idx)}>></AmCheckBox>
                                                    </FormInline></div>}
                                                </FormInline>
                                            </FormInline> : null}
                                            {props.random === true ? <FormInline style={{ marginLeft: "15px" }}>
                                                <LabelH>{t("Counting")} (%):</LabelH>
                                                <AmInput
                                                    defaultValue={x.Randoms ? x.Randoms : qtyrandom ? qtyrandom : x.Random ? x.Random : "100"}
                                                    style={{ width: "50px", marginLeft: "5px" }}
                                                    type="number"
                                                    onChange={(ele) => { onChangeRandoms(ele, idx) }}
                                                >
                                                </AmInput>
                                                <label>%</label>
                                            </FormInline> : null}

                                            {props.dataSortShow === true ? < FormInline>
                                                <div style={{ marginLeft: "15px", paddingTop: "10px" }}> <LabelH>{t("Sorting")}</LabelH></div>
                                                <AmButton styleType="add_outline" style={{ marginLeft: "880px", width: "150px" }}
                                                    onClick={() => { setDialogSort(true); setAddData(true); setTitleSort("Add Sorting"); setindexBtn(idx); }}>
                                                    {t('Add Sorting')}</AmButton>
                                            </FormInline> : null}
                                            {props.dataSortShow === true ? <div style={{ marginTop: "5px" }}><AmTable
                                            ...
                                            ></AmTable></div> : null}
                                            < FormInline>
                                                <div style={{ marginLeft: "15px" }}><LabelH>{t("Coditions")}</LabelH></div>
                                                {props.docType !== "audit" ? <div><AmButton styleType="add_outline" style={{ marginLeft: "880px", width: "150px" }}
                                                    onClick={() => { setDialog(true); setAddData(true); setTitle("Add"); setindexBtn(idx); }}>
                                                    {t('Add Conditions')}</AmButton></div> : null}
                                            </FormInline>
                                            <div style={{ paddingBottom: "10px" }}><AmTable
                                                data={dataSource[idx] === undefined ? [] : dataSource[idx]}
                                                reload={reload}
                                                columns={columnConditionx}
                                                minRows={1}
                                                sortable={false}
                                            ></AmTable></div></Paper> : null}</Collapse></Card></BorderAdd></div>
                            })
                        }</div> : null}
                                ...
                                <Grid item >
                                    <div style={{ marginRight: "10px" }}>
                                        <AmButton styleType="add"
                                            onClick={addConPage}>{'Add Queue'}</AmButton>
                                    </div>
                                </Grid></Grid></div>
                    </div> : null}
            </Card> : null}

            {value === 1 ? <div>{datasDoc !== [] ? <Card style={{ overflow: "initial" }}>
                            ...
                            return <div>
                                                    ...
                                                    <Grid item xs container direction="column" spacing={16}>
                                                        <div style={{ marginLeft: "15px" }}>
                                                            <FormInline>
                                                                ...
                                                                <LabelHDoc>{x.DataDocumentCode}</LabelHDoc>
                                                                <AmRediRectInfo
                                                                    link={false}
                                                                    textLink={docCodelink}
                                                                    api={props.apidetail + x.DataDocumentID} history={props.history}>
                                                                </AmRediRectInfo >
                                                            </FormInline>
                                                        </div></Grid>
                                                    <Grid item>
                                                        <AmButton style={{ width: "100px", marginRight: "20px" }} styleType="confirm_outline"
                                                            onClick={() => { OnclickBackAddDocument({ ...docs }, dataSOs, dataSorts) }}
                                                        >Back</AmButton></Grid></Grid>
                                            </ FormInline>
                                        </div></FormInline>
                                </div>
                                <div style={{ paddingBottom: "10px", paddingTop: "10px" }}>
                                    {
                                        x.DataDocdetail.map((x) => {
                                            return <div style={{ marginTop: "10px", }}><Grid container spacing={24}>
                                                {x.map(y => {
                                                    return <Grid xs={6} style={{ paddingLeft: "10px", paddingBottom: "10px" }}>
                                                        <FormInline>
                                                            <div style={{ paddingLeft: "10px", width: "200px" }}><LabelH>{t(y.label)}</LabelH></div>
...

                                {
                                    x.DataDocumentItem.map((y, idxItem) => {
                                        ...
                                        if (datasDoc[idx]["DataSource"][idxItem] !== undefined)
                                            var dataSO = datasDoc[idx]["DataSource"][idxItem]
                                        if (datasDoc[idx]["DataSorting"][idxItem] !== undefined)
                                            var dataSort = datasDoc[idx]["DataSorting"][idxItem]

                                        return <div style={{ marginLeft: "10px", marginRight: "10px" }}>
                                            <BorderQueu> <Card>
                                                <AmButton styleType="info_clear" style={{ marginLeft: "10px", }}
                                                    onClick={() => onclickToggelDataQueue(idx + 'T' + idxItem)}>{y.Code}: {y.SKUMaster_Name}
                                                    <ExpandLessIcon className={toggleQueue[idx + 'T' + idxItem] ? classes.expand : classes.collapse} /></AmButton>

                                                <div style={{ clear: "both" }}></div>
                                                <Collapse in={toggleQueue[idx + 'T' + idxItem]}>
                                                    <div>
                                                        {toggleQueue[idx + 'T' + idxItem] === true ? <Paper elevation={4} className={classes.paper}>
                                                            <FormInline style={{ marginTop: "10px", marginLeft: "15px" }}>
                                                                <div style={{ width: "800px" }}> <LabelH>{y.Code}  : {y.SKUMaster_Name}
                                                                </LabelH>
                                                                    {y.palletcode ? <LabelH> / {y.palletcode}</LabelH> : null}
                                                                    {y.locationcode ? <LabelH> / {y.locationcode}</LabelH> : null}
                                                                </div>
                                                                < div >
                                                                    <LabelH>{t("Qty")} :</LabelH> <label>{y.Quantity} {y.UnitType_Name}({y.BaseqtyMax} {y.BaseUnitType_Code} ) </label></ div>

                                                            </FormInline >

                                                            <FormInline style={{ marginTop: "10px", marginLeft: "15px" }}>
                                                                <div style={{ width: "200px" }}><LabelH>{t("Priority")} :</LabelH></div>
                                                                <label style={{ marginLeft: "15px" }}> {y.PriorityLabel ? y.PriorityLabel : "Normal"} </label>
                                                            </FormInline>
                                                            {props.advanceCondition === true ?
                                                                < FormInline style={{ marginTop: "10px", marginLeft: "15px" }}>
                                                                    <div style={{ width: "200px" }}><LabelH>{t("Advance Condition")} : </LabelH></div>
                                                                    <label style={{ marginLeft: "15px" }}>{y.FullPallet === true ? "FullPallet" : ""}</label>
                                                                    <label style={{ marginLeft: "15px" }}>{y.ShelfLifeDate === true ? "ShelfLifeDate" : ""}</label>
                                                                    <label style={{ marginLeft: "15px" }}>{y.IncubateDate === true ? "IncubateDate" : ""}</label>
                                                                    <label style={{ marginLeft: "15px" }}>{y.ExpireDate === true ? "ExpireDate" : ""}</label>

                                                                </FormInline> : null}
                                                            {props.status === true ? <FormInline style={{ marginTop: "10px", marginLeft: "15px" }}>
                                                                <div style={{ width: "200px" }}>< LabelH >{t("Status")} :</LabelH></div>

                                                                <label style={{ marginLeft: "15px" }}>{y.Receive === true ? "Recive" : ""}</label>
                                                                <label style={{ marginLeft: "15px" }}>{y.Block === true ? "Block" : ""}</label>
                                                                <label style={{ marginLeft: "15px" }}>{y.QC === true ? "QC" : ""}</label>
                                                            </FormInline> : null}

                                                            {props.random === true ? <FormInline style={{ marginTop: "10px", marginLeft: "15px" }}>
                                                                <div style={{ width: "200px" }}><LabelH>{t("Counting")} (%) : </LabelH></div>
                                                                <label style={{ height: "10px", marginLeft: "15px" }}>{y.Randoms ? y.Randoms + "%" : y.Random ? y.Random + "%" : "-"}</label>
                                                            </FormInline> : null}
                                                            {props.docType !== "audit" ? <div style={{ marginLeft: "15px", paddingTop: "10px", width: "200px" }}>
                                                                <LabelH>{t("Sorting")}</LabelH></div> : null}
                                                            {props.docType !== "audit" ? <div style={{ marginTop: "5px" }}><AmTable
                                                                data={dataSort === undefined ? [] : dataSort}
                                                                reload={reload}
                                                                columns={columnSort}
                                                                minRows={1}
                                                                sortable={false}
                                                            ></AmTable></div> : null}
                                                            <div style={{ marginLeft: "15px", paddingTop: "10px", width: "200px" }}><LabelH>{t("Coditions")}</LabelH></div>
                                                            <div style={{ marginTop: "5px" }}><AmTable
                                                                data={dataSO === undefined ? [] : dataSO}
                                                                reload={reload}
                                                                columns={columnCondition}
                                                                minRows={1}
                                                                sortable={false}
                                                            ></AmTable></div>
                                                        </Paper> : null}</div></Collapse></Card></BorderQueu></div>
                                    })
                                }
                            </div>
                        })
                        ...
                        <Grid container spacing={24}>
                            ...
                            <Grid item >
                                <FormInline style={{ marginRight: "10px" }}>
                                    ...
                                    <AmButton style={{ width: "80px", marginLeft: "5px" }} styleType="confirm"
                                        onClick={() => { OnclickConfirmQueue() }}
                                    >Process</AmButton>
                                    <AmButton style={{ width: "80px", marginLeft: "5px" }} styleType="delete"
                                        onClick={() => { OnclickConfirmCleare() }}
                                    >Clear</AmButton>
                                </FormInline>
                            </Grid></Grid> </FormInline> : null}
                </div></Card> : null}</div> : null}

            <AmEditorTable style={{ width: "600px", height: "500px" }} titleText={title} open={dialog}
                columns={editorListcolunmCondition()} data={editData}
                onAccept={(status, rowdata) => onHandleEditConfirm(status, rowdata)}  ></AmEditorTable>
            <AmEditorTable style={{ width: "600px", height: "500px" }} titleText={titleSort} open={dialogSort}
                columns={editorListcolunmSort()} data={editDataSort}
                onAccept={(status, rowdata) => onChangeEditorSortConfirm(status, rowdata)}  ></AmEditorTable>
            ...
            <AmDialogConfirm
                ...
                customAcceptBtn={<AmButton styleType="confirm_clear" onClick={() => { OnConfirmprocess() }}>{t("OK")}</AmButton>}
